 ...
 * CiscoBfdSessUp.php
 *
 * Handler for BFD session up traps (Cisco).
 * This handler processes the ciscoBfdSessUp event
 *
 * -
 *
...
class CiscoBfdSessUp implements SnmptrapHandler
{
    /**
     * Handle snmptrap ciscoBfdSessUp.
     *
     * @param  Device  $device
     * @param  Trap  $trap
     * @return void
    ...
    public function handle(Device $device, Trap $trap)
    {
        ...
        if (! $port) {
            ...
            Log::warning("Snmptrap ciscoBfdSessUp: Could not find port at ifIndex $ifIndex for device: " . $device->hostname);

            // log the Trap in the device's Eventlog (without associating a specific port)
            $trap->log("BFD Session Up (ifIndex: $ifIndex). Motivo: $diag", Severity::Ok);

            return;
        }

        // Build the message for a known port
        $message = "Cisco BFD Up Trap: $port->ifDescr. Reason: $diag";

        // Log the Trap in the Eventlog directly associated with the interface (port_id)
        $trap->log($message, Severity::Ok, 'interface', $port->port_id);
    }
}
